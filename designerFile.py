# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file '.\QtDesignerUI.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(980, 712)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.startButton = QtWidgets.QPushButton(self.centralwidget)
        self.startButton.setGeometry(QtCore.QRect(20, 490, 121, 51))
        self.startButton.setObjectName("startButton")
        self.stopTrackingButton = QtWidgets.QPushButton(self.centralwidget)
        self.stopTrackingButton.setGeometry(QtCore.QRect(20, 550, 121, 51))
        self.stopTrackingButton.setObjectName("stopTrackingButton")
        self.startingAzimuthBox = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.startingAzimuthBox.setGeometry(QtCore.QRect(570, 240, 251, 31))
        self.startingAzimuthBox.setObjectName("startingAzimuthBox")
        self.startingElevationBox = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.startingElevationBox.setGeometry(QtCore.QRect(570, 290, 251, 31))
        self.startingElevationBox.setObjectName("startingElevationBox")
        self.label_4 = QtWidgets.QLabel(self.centralwidget)
        self.label_4.setGeometry(QtCore.QRect(480, 250, 79, 16))
        self.label_4.setObjectName("label_4")
        self.label_5 = QtWidgets.QLabel(self.centralwidget)
        self.label_5.setGeometry(QtCore.QRect(470, 300, 85, 16))
        self.label_5.setObjectName("label_5")
        self.calibrateButton = QtWidgets.QPushButton(self.centralwidget)
        self.calibrateButton.setGeometry(QtCore.QRect(830, 290, 105, 23))
        self.calibrateButton.setObjectName("calibrateButton")
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(410, 10, 121, 16))
        self.label.setObjectName("label")
        self.GSLatBox = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.GSLatBox.setGeometry(QtCore.QRect(560, 10, 241, 31))
        self.GSLatBox.setObjectName("GSLatBox")
        self.label_7 = QtWidgets.QLabel(self.centralwidget)
        self.label_7.setGeometry(QtCore.QRect(10, 100, 141, 16))
        self.label_7.setObjectName("label_7")
        self.COMPortComboBox = QtWidgets.QComboBox(self.centralwidget)
        self.COMPortComboBox.setGeometry(QtCore.QRect(10, 130, 151, 20))
        self.COMPortComboBox.setObjectName("COMPortComboBox")
        self.COMPortComboBox.addItem("")
        self.connectToArduinoButton = QtWidgets.QPushButton(self.centralwidget)
        self.connectToArduinoButton.setGeometry(QtCore.QRect(190, 130, 98, 23))
        self.connectToArduinoButton.setObjectName("connectToArduinoButton")
        self.tiltUpButton = QtWidgets.QPushButton(self.centralwidget)
        self.tiltUpButton.setGeometry(QtCore.QRect(170, 262, 81, 31))
        self.tiltUpButton.setObjectName("tiltUpButton")
        self.panLeftButton = QtWidgets.QPushButton(self.centralwidget)
        self.panLeftButton.setGeometry(QtCore.QRect(110, 300, 81, 31))
        self.panLeftButton.setObjectName("panLeftButton")
        self.panRightButton = QtWidgets.QPushButton(self.centralwidget)
        self.panRightButton.setGeometry(QtCore.QRect(230, 300, 81, 31))
        self.panRightButton.setObjectName("panRightButton")
        self.tiltDownButton = QtWidgets.QPushButton(self.centralwidget)
        self.tiltDownButton.setGeometry(QtCore.QRect(170, 340, 81, 31))
        self.tiltDownButton.setObjectName("tiltDownButton")
        self.confirmGSLocationButton = QtWidgets.QPushButton(self.centralwidget)
        self.confirmGSLocationButton.setGeometry(QtCore.QRect(830, 120, 85, 23))
        self.confirmGSLocationButton.setObjectName("confirmGSLocationButton")
        self.GPSRequestButton = QtWidgets.QPushButton(self.centralwidget)
        self.GPSRequestButton.setGeometry(QtCore.QRect(830, 10, 107, 23))
        self.GPSRequestButton.setObjectName("GPSRequestButton")
        self.IMEIComboBox = QtWidgets.QComboBox(self.centralwidget)
        self.IMEIComboBox.setGeometry(QtCore.QRect(10, 50, 151, 20))
        self.IMEIComboBox.setObjectName("IMEIComboBox")
        self.confirmIMEIButton = QtWidgets.QPushButton(self.centralwidget)
        self.confirmIMEIButton.setGeometry(QtCore.QRect(190, 50, 75, 23))
        self.confirmIMEIButton.setObjectName("confirmIMEIButton")
        self.label_6 = QtWidgets.QLabel(self.centralwidget)
        self.label_6.setGeometry(QtCore.QRect(10, 20, 127, 16))
        self.label_6.setObjectName("label_6")
        self.label_2 = QtWidgets.QLabel(self.centralwidget)
        self.label_2.setGeometry(QtCore.QRect(410, 60, 131, 20))
        self.label_2.setObjectName("label_2")
        self.GSLongBox = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.GSLongBox.setGeometry(QtCore.QRect(560, 60, 241, 31))
        self.GSLongBox.setObjectName("GSLongBox")
        self.label_3 = QtWidgets.QLabel(self.centralwidget)
        self.label_3.setGeometry(QtCore.QRect(410, 110, 141, 20))
        self.label_3.setObjectName("label_3")
        self.GSAltBox = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.GSAltBox.setGeometry(QtCore.QRect(560, 110, 241, 31))
        self.GSAltBox.setObjectName("GSAltBox")
        self.distanceDisplay = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.distanceDisplay.setGeometry(QtCore.QRect(560, 380, 251, 31))
        self.distanceDisplay.setObjectName("distanceDisplay")
        self.azimuthDisplay = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.azimuthDisplay.setGeometry(QtCore.QRect(560, 430, 251, 31))
        self.azimuthDisplay.setObjectName("azimuthDisplay")
        self.elevationDisplay = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.elevationDisplay.setGeometry(QtCore.QRect(560, 480, 251, 31))
        self.elevationDisplay.setObjectName("elevationDisplay")
        self.errorMessageBox = QtWidgets.QPlainTextEdit(self.centralwidget)
        self.errorMessageBox.setGeometry(QtCore.QRect(460, 560, 371, 61))
        self.errorMessageBox.setObjectName("errorMessageBox")
        self.label_8 = QtWidgets.QLabel(self.centralwidget)
        self.label_8.setGeometry(QtCore.QRect(410, 380, 131, 21))
        self.label_8.setObjectName("label_8")
        self.label_9 = QtWidgets.QLabel(self.centralwidget)
        self.label_9.setGeometry(QtCore.QRect(490, 430, 47, 13))
        self.label_9.setObjectName("label_9")
        self.label_10 = QtWidgets.QLabel(self.centralwidget)
        self.label_10.setGeometry(QtCore.QRect(490, 490, 47, 13))
        self.label_10.setObjectName("label_10")
        self.label_11 = QtWidgets.QLabel(self.centralwidget)
        self.label_11.setGeometry(QtCore.QRect(390, 580, 71, 20))
        self.label_11.setObjectName("label_11")
        self.bigTiltUpbutton = QtWidgets.QPushButton(self.centralwidget)
        self.bigTiltUpbutton.setGeometry(QtCore.QRect(170, 230, 81, 31))
        self.bigTiltUpbutton.setObjectName("bigTiltUpbutton")
        self.bigPanRightButton = QtWidgets.QPushButton(self.centralwidget)
        self.bigPanRightButton.setGeometry(QtCore.QRect(320, 300, 81, 31))
        self.bigPanRightButton.setObjectName("bigPanRightButton")
        self.bigTiltDownButton = QtWidgets.QPushButton(self.centralwidget)
        self.bigTiltDownButton.setGeometry(QtCore.QRect(170, 380, 81, 31))
        self.bigTiltDownButton.setObjectName("bigTiltDownButton")
        self.bigPanLeftButton = QtWidgets.QPushButton(self.centralwidget)
        self.bigPanLeftButton.setGeometry(QtCore.QRect(24, 300, 81, 31))
        self.bigPanLeftButton.setObjectName("bigPanLeftButton")
        self.calculateStartingPosButton = QtWidgets.QPushButton(self.centralwidget)
        self.calculateStartingPosButton.setGeometry(QtCore.QRect(830, 240, 141, 21))
        self.calculateStartingPosButton.setObjectName("calculateStartingPosButton")
        self.label_12 = QtWidgets.QLabel(self.centralwidget)
        self.label_12.setGeometry(QtCore.QRect(840, 180, 121, 20))
        self.label_12.setObjectName("label_12")
        self.label_13 = QtWidgets.QLabel(self.centralwidget)
        self.label_13.setGeometry(QtCore.QRect(840, 200, 101, 16))
        self.label_13.setObjectName("label_13")
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 980, 21))
        self.menubar.setObjectName("menubar")
        self.menuBRAD_Station_Tracking_Software = QtWidgets.QMenu(self.menubar)
        self.menuBRAD_Station_Tracking_Software.setObjectName("menuBRAD_Station_Tracking_Software")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)
        self.menubar.addAction(self.menuBRAD_Station_Tracking_Software.menuAction())

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.startButton.setText(_translate("MainWindow", "start tracking"))
        self.stopTrackingButton.setText(_translate("MainWindow", "Stop Tracking"))
        self.label_4.setText(_translate("MainWindow", "Starting Azimuth"))
        self.label_5.setText(_translate("MainWindow", "Starting Elevation"))
        self.calibrateButton.setText(_translate("MainWindow", "Set Starting Position"))
        self.label.setText(_translate("MainWindow", "Ground Station Latitude: "))
        self.label_7.setText(_translate("MainWindow", "Select Arduino from list below"))
        self.COMPortComboBox.setItemText(0, _translate("MainWindow", "com port"))
        self.connectToArduinoButton.setText(_translate("MainWindow", "connect to arduino"))
        self.tiltUpButton.setText(_translate("MainWindow", "Tilt Up"))
        self.panLeftButton.setText(_translate("MainWindow", "Pan Left"))
        self.panRightButton.setText(_translate("MainWindow", "Pan Right"))
        self.tiltDownButton.setText(_translate("MainWindow", "Tilt Down"))
        self.confirmGSLocationButton.setText(_translate("MainWindow", "Set Coordinates"))
        self.GPSRequestButton.setText(_translate("MainWindow", "Request GPS Coords"))
        self.confirmIMEIButton.setText(_translate("MainWindow", "Confirm IMEI"))
        self.label_6.setText(_translate("MainWindow", "Enter or Select IMEI below"))
        self.label_2.setText(_translate("MainWindow", "Ground Station Longitude:"))
        self.label_3.setText(_translate("MainWindow", "Ground Station Altitude (m):"))
        self.label_8.setText(_translate("MainWindow", "Line of Sight Distance (km): "))
        self.label_9.setText(_translate("MainWindow", "Azimuth:"))
        self.label_10.setText(_translate("MainWindow", "Elevation:"))
        self.label_11.setText(_translate("MainWindow", "Status box"))
        self.bigTiltUpbutton.setText(_translate("MainWindow", "Tilt Up (big)"))
        self.bigPanRightButton.setText(_translate("MainWindow", "Pan Right (big)"))
        self.bigTiltDownButton.setText(_translate("MainWindow", "Tilt Down (big)"))
        self.bigPanLeftButton.setText(_translate("MainWindow", "Pan Left (big)"))
        self.calculateStartingPosButton.setText(_translate("MainWindow", "Calculate Starting Position"))
        self.label_12.setText(_translate("MainWindow", "When pointed at sun,"))
        self.label_13.setText(_translate("MainWindow", "click button below"))
        self.menuBRAD_Station_Tracking_Software.setTitle(_translate("MainWindow", "BRAD Station Tracking Software"))


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
